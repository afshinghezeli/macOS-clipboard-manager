...
- [ ] M0.9 Sandbox spike for [ADR 0002](adr/0002-ship-a-sandboxed-developer-id-app.md): a sandboxed dev build (1) posts ⌘V into TextEdit and (2) updates itself with Sparkle from a local appcast. Accept or supersede the ADR based on the result. Part 2 passed on 2026-09-22: a sandboxed build without the network entitlement fetched a local appcast through Sparkle's Downloader service, checked the EdDSA signature and replaced itself with the new build after quitting. Part 1 needs the owner to grant the paste permission.
...
- [x] M2.3 Fuzzy fallback over a small in-memory pool when exact matches are scarce.
...
- [ ] M5.1 App icon, drawn by hand. Needs the owner. `make icon PNG=path/to/icon-1024.png` turns the artwork into `Support/AppIcon.icns`.
